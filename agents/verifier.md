...
5. An toàn (luật K7, K4 trong `an-toan/LUAT-CHUNG.md`): quét bí mật trong diff và file mới (`gitleaks detect` nếu có; không thì grep mẫu khóa/token, chỉ báo vị trí, không in giá trị); package mới có trong `DEPENDENCIES.md` và ghim phiên bản chưa; `.env.example` còn khớp biến mà code đọc; kiểm soát nào (xác thực, validate, rate limit, CORS, RLS, test) bị tắt hay nới lỏng (K9). Được đưa mã luật cụ thể thì kiểm từng luật đó: ĐẠT / CHƯA ĐẠT kèm bằng chứng. Không đọc `.env` thật (K1).
...
5. **An toàn**: từng luật được giao → ĐẠT/CHƯA ĐẠT + bằng chứng; kết quả quét bí mật; hoặc "không đổi" nếu việc không chạm an toàn
6. **Lệch so với plan / spec**
7. **Đã chạy**: lệnh + kết quả rút gọn
